    ...
    for name, w in model.named_parameters():
        if exclude not in name:
            ...
            if 'weight' in name:
                ...
                pass
# class_weights = torch.tensor([0.1, 0.9], device='cuda')  # 类别权重处理不均衡数据
...
def train(config, model, train_iter, dev_iter, test_iter):
    ...
    for layer in model.modules():
        if isinstance(layer, torch.nn.BatchNorm2d) or isinstance(layer, torch.nn.BatchNorm1d):
            layer.eval()  # 将其切换为 eval 模式（禁用训练更新）
            layer.requires_grad_(False)  # 禁止更新权重
...
def evaluate(config, model, data_iter, test=False):
    ...
    with torch.no_grad():
        ...
        #         # 输出预测错误的文本
        #         print(f"Prediction error at index {i}, Text: {texts[i]}")
