...
class Config(object):
    ...
    def __init__(self, dataset):
        self.model_name = 'ERNIE_RCNN_roberta_chinese_improved'
        self.train_path = dataset + '/data/train.txt'  # 训练集
        self.dev_path = dataset + '/data/dev.txt'      # 验证集
        self.test_path = dataset + '/data/test.txt'    # 测试集
        self.class_list = [x.strip() for x in open(dataset + '/data/class.txt', encoding='utf-8').readlines()]  # 类别
        ...
        self.batch_size = 32  # 小批量样本数
        self.pad_size = 64  # 每句话处理成的长度(短填长切)
        self.learning_rate = 5e-5  # 学习率
        self.bert_path = './chinese_roberta_wwm_ext_pytorch'  # BERT路径
        self.tokenizer = BertTokenizer.from_pretrained(self.bert_path)  # 加载BERT的分词器
        self.hidden_size = 768  # 隐藏层大小
        self.filter_sizes = (2, 3, 4)  # 卷积核尺寸
        self.num_filters = 256  # 卷积核数量(channels数)
        self.dropout = 0.3  # Dropout防止过拟合
        self.rnn_hidden = 512  # LSTM隐藏层大小
